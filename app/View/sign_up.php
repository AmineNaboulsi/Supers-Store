<?php
  ...
  if(isset($_COOKIE['validation'])){
    ...
    if($validation == 'valide'){
        ...
        align-items: center;
        gap: 0.9rem;
        animation: slideFromLeft 2.3s ease;
  ...
  if(isset($_POST["sign_up"])) {
    ...
    if($res->num_rows >0){
        setcookie("validation" , "novalide");

    }else{
        mysqli_query($con , "insert into user values('$firstname','$lastname','$email','$password','custumer');");
        setcookie("validation" , "valide");
    }
...
<body class="font-Inter select-none text-white">
    ...
    <section>
        <form method="post" enctype="multipart/form-data" onsubmit="done()" >
        ...
        <div class="bg-200 shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px] mx-2 md:mx-20 lg:mx-auto lg:max-w-[800px] my-10 px-10 py-10 flex flex-col items-center text-center gap-5 rounded-md">
            <div>               
                <h1 name="signin" class="text-2xl font-bold text-900">SIGN IN</h1>             
            </div>
            <div class="flex flex-col gap-5 ">
                <div class="relative">
                    <input required name="firstname" class="py-2 px-12 rounded-md bg-100 placeholder:text-900/50 text-900 outline-none" type="text" placeholder="First Name">
                    <i class="fa-solid absolute top-[10px] left-[14px] text-900 text-xl fa-user"></i>
                </div>
                <div class="relative">
                    <input required name="lastname" class="py-2 px-12 rounded-md bg-100 placeholder:text-900/50 text-900 outline-none" type="text" placeholder="Last Name">
                    <i class="fa-solid absolute top-[10px] left-[14px] text-900 text-xl fa-user"></i>
                </div>
                <div class="relative">
                ...
                <div class="relative">
                    <input required  class="py-2 px-12 rounded-md bg-100 placeholder:text-900/50 text-900 outline-none" type="password" placeholder="Confirm Password">
                    <i class="fa-solid  absolute top-[10px] left-[14px] text-900 text-xl fa-lock"></i>
                </div>
                <div class="w-full flex justify-between items-center">
                    <button  class="w-full bg-500 hover:bg-600 font-semibold py-3 px-10 rounded-md" type="submit"  name="sign_up" id="sign_up"> SIGN IN</button>
                </div>
            </div>
        </div>
        </form>
